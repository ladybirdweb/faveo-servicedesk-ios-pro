...
@interface SearchViewController ()<MultiSelectSegmentedControlDelegate,UITextFieldDelegate>
{
    Utils *utils;
...
-(void)multiSelect:(MultiSelectSegmentedControl *)multiSelecSegmendedControl didChangeValue:(BOOL)value atIndex:(NSUInteger)index{
    if([_seachTextField.text isEqualToString:@""]){
        ...
        if(index==0)
        {
            ...
            [_seachTextField resignFirstResponder];
            [SVProgressHUD showWithStatus:@"Getting Ticket data"];
        ...
        if (index==1) {
            ...
            [_seachTextField resignFirstResponder];
            [SVProgressHUD showWithStatus:@"Getting user data"];
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView==_tableview1)
    {
        ...
        globalVariables.ticketStatus=[statusValueDict objectForKey:@"name"];
        ...
        {
            if (![Utils isEmpty:fname] && ![Utils isEmpty:lname])
            {    globalVariables.firstNameFromTicket=fname;
                globalVariables.lastNameFromTicket=lname;
            }
            else if (![Utils isEmpty:fname]){
                globalVariables.firstNameFromTicket=[NSString stringWithFormat:@"%@",fname];
        ...
        else
        {
            if(![Utils isEmpty:userName])
            {
                globalVariables.firstNameFromTicket=userName;
                globalVariables.lastNameFromTicket=@"";
            }
            else{
                globalVariables.firstNameFromTicket=NSLocalizedString(@"", nil);
                globalVariables.lastNameFromTicket=@"";
            }
        ...
        if  (![Utils isEmpty:ticketNumber] && ![ticketNumber isEqualToString:@""])
        {
            globalVariables.ticketNumber=ticketNumber;
        }
        else
        {
            globalVariables.ticketNumber=NSLocalizedString(@"", nil);
        }
        
    
        globalVariables.ticketId=(NSNumber*)ticketidIs;
    ...
    else if(tableView==_tableview2)
    {
        ...
        if(![Utils isEmpty:userName])
        {
            globalVariables.userNameFromUserList=userName;
        }else{
            globalVariables.userNameFromUserList=@"Not Available";
        }
        ...
        {
            if (![Utils isEmpty:fname] && ![Utils isEmpty:lname])
            {   globalVariables.First_nameFromUserList=fname;
                globalVariables.Last_nameFromUserList=lname;
            }
            else if (![Utils isEmpty:fname]){
                globalVariables.First_nameFromUserList=fname;
                globalVariables.Last_nameFromUserList=@"";
            }
        }
        else
        {
            if(![Utils isEmpty:userName])
            {
                globalVariables.First_nameFromUserList=userName;
                globalVariables.Last_nameFromUserList=@"";
            }
            else{
                globalVariables.First_nameFromUserList=@"";
                globalVariables.Last_nameFromUserList=@"";
            }
            
        }
        
        globalVariables.userImageFromUserList=profilPic;
        globalVariables.customerFromView=@"normalView";
        globalVariables.userRoleFromUserList=@"";
        globalVariables.userIDFromUserList=userID1;
        ...
        {
            globalVariables.emailFromUserList=@"";
        }
        globalVariables.phoneNumberFromUserList=@"";
        globalVariables.mobileNumberFromUserList=@"";
        globalVariables.userStateFromUserList=@"1";
        globalVariables.mobilecodeFromUserList=@"";
        //
        ClientDetailsViewController *td=[self.storyboard instantiateViewControllerWithIdentifier:@"clientDetailsViewId"];
        [self.navigationController pushViewController:td animated:YES];
